...
does the same with a small **neural network** — reaching similar accuracy with
both approaches.
...
*White trail: true path. Cyan trail: tracked path. When the particle is bright
...
the particle dimmer is as simple as lowering one number — which is how the two
GIFs above were produced.
...
Doing this frame by frame gives the full trajectory. It uses only base MATLAB —
no toolboxes.
...
refinement differs — fitting vs. learning.
...
well — a nice confirmation that the learned model recovers what the classical
method does, trained purely on simulated data.
